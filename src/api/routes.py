...
async def get_agent(agent_id: str):
    normalized_id = _normalized_agent_id_or_422(agent_id)
...
async def delete_agent(agent_id: str):
    normalized_id = _normalized_agent_id_or_422(agent_id)
    if not registry.delete(normalized_id):
        raise HTTPException(status_code=404, detail="Agent not found")
...
async def start_agent(agent_id: str):
    normalized_id = _normalized_agent_id_or_422(agent_id)
    if not registry.update_status(normalized_id, AgentStatus.RUNNING):
        raise HTTPException(status_code=404, detail="Agent not found")
...
async def stop_agent(agent_id: str):
    normalized_id = _normalized_agent_id_or_422(agent_id)
    if not registry.update_status(normalized_id, AgentStatus.PAUSED):
        raise HTTPException(status_code=404, detail="Agent not found")
    return {"status": "stopped"}


def _normalized_agent_id_or_422(agent_id: str) -> str:
    try:
        return normalize_agent_id(agent_id)
    except AgentIdValidationError:
        raise HTTPException(status_code=422, detail="Malformed agent ID")
