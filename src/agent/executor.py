...
class AgentExecutor:
    def __init__(
        ...
        if result_ttl_seconds is not None and (
            isinstance(result_ttl_seconds, bool) or result_ttl_seconds <= 0
        ):
            raise ValueError("result_ttl_seconds must be positive or None")
        ...
        self._results: "OrderedDict[str, Tuple[float, Any]]" = OrderedDict()
    ...
    def get_result(self, execution_id: str) -> Optional[Any]:
        self._prune_results()
        entry = self._results.get(execution_id)
        if entry is None:
            return None
        return entry[1]
    ...
    def _store_result(self, execution_id: str, result: Any) -> None:
        self._prune_results()
        self._results[execution_id] = (time.time(), result)
    ...
    def _prune_results(self) -> None:
        if self.result_ttl_seconds is None:
            return
        expires_before = time.time() - self.result_ttl_seconds
        expired = [
            execution_id
            for execution_id, (stored_at, _) in self._results.items()
            if stored_at < expires_before
        ]
