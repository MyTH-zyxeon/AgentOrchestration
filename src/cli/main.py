...
def cli():
    ...
    if args.command == "init":
        ...
        try:
            verification = deploy_manifest(
                args.manifest,
                credential_loader=load_environment_credentials,
            )
        except DeploymentVerificationError as exc:
            print(f"Deployment rejected: {exc}", file=sys.stderr)
            sys.exit(2)
        print(f"Deployment bundle verified: {verification.bundle_digest}")
